 ...
 setQrImageUrl:any;
  ...
  constructor(private router: Router, private route: ActivatedRoute,  private dataservice:DataService, private sanitizer:DomSanitizer ) {
   ...
   this.imageString=this.route.snapshot.params['setQrImageUrl'];
    this.setQrImageUrl=this.sanitizer.bypassSecurityTrustUrl(this.imageString+'data:image/PNG;base64,');
  ...
  authenticateOtp(){
    ...
    if (!isNullOrUndefined(sessionStorage.getItem('userName'))) {
      ...
      this.dataservice.authenticateOtp(userName, this.otpCode).subscribe(response =>{
  ...
  ngOnInit() :void{
    ...
    if (!isNullOrUndefined(sessionStorage.getItem('userName'))) {
      ...
      userName: new FormControl(this.otpCode, [
        Validators.required,
